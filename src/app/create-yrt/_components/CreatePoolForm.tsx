...
export function CreatePoolForm({
  ...
  useEffect(() => {
    if (isSuccess && hash) {
      toast.success("DEX Pool created successfully!", {
        description: (
  ...
  useEffect(() => {
    if (error) {
      toast.error("Failed to create DEX Pool", {
        description: error.message,
      });
    }
  }, [error]);

  // Auto-fill YRT token address when provided
  useEffect(() => {
    if (createdYrtAddress && formData.yrtTokenAddress !== createdYrtAddress) {
      const event = {
        target: {
          name: "yrtTokenAddress",
          value: createdYrtAddress,
        },
      } as React.ChangeEvent<HTMLInputElement>;
      onInputChange(event);
    }
  }, [createdYrtAddress, formData.yrtTokenAddress, onInputChange]);
      ...
      <form onSubmit={onSubmit} className="space-y-6">
        <div className="space-y-4">
          ...
          <div>
            ...
            <input
              type="text"
              name="yrtTokenAddress"
              value={formData.yrtTokenAddress}
              onChange={onInputChange}
              placeholder="0x..."
              className="w-full px-4 py-3 bg-[#111111] border border-[#2A2A2A] rounded-lg text-white placeholder-gray-500 focus:border-white focus:outline-none font-mono text-sm"
              required
            />
            <p className="text-xs text-gray-500 mt-1">
              Address of the YRT token created in Step 1
            </p>
          </div>
          ...
          <div>
            ...
            />
            <p className="text-xs text-gray-500 mt-1">
              Token to pair with YRT for trading
            </p>
          </div>
          ...
          <div>
            ...
            />
            <p className="text-xs text-gray-500 mt-1">
              Display name for the liquidity pool
            </p>
          </div>
          ...
          <div>
            ...
            />
            <p className="text-xs text-gray-500 mt-1">
              Address that can withdraw/inject funds from the pool
            </p>
          </div>
        </div>
        ...
        <div className="flex justify-end pt-6">
          ...
          >
            {isLoading ? (
              ...
              <>
                <span className="cursor-pointer">Create DEX Pool</span>
              </>
            )}
          </Button>
        </div>
        ...
        {!address && (
          <p className="text-center text-gray-300 text-sm">
            Please connect your wallet to create a DEX pool
          </p>
        )}
      </form>
    </div>
